    ...
    def __init__(self,
        ...
        with tf.variable_scope('embed') as s:
            ...
            with tf.variable_scope('project') as s:
                project = lambda x: op.forward(x, dim=self.project_dim, scope=s,
                        reuse=tf.AUTO_REUSE)
        ...
        with tf.variable_scope('compare') as s:
            v1 = op.forward(tf.concat([x1, beta ], 2), scope=s, **self._fc_opts)
            v2 = op.forward(tf.concat([x2, alpha], 2), scope=s, **self._fc_opts)

        with tf.variable_scope('aggregate') as s:
            v1 = tf.reduce_sum(v1 * mask1, axis=1)
            v2 = tf.reduce_sum(v2 * mask2, axis=1)
            y_hat = op.forward(tf.concat([v1, v2], 1), dim=class_num, scope=s)
